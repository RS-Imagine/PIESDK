...
namespace PIE_We
{
    public partial class FormBandMergeTemperature : UIForm
    {
        public FormBandMergeTemperature()
        {
            InitializeComponent();
        }
        ...
        private void uiButtonRun_Click(object sender, EventArgs e)
        {
            if (uiTextBoxIn.Enabled == true)
            {
                ...
                UIMessageBox.Show("月温度数据成功合成至年数据！", "提示", UIStyle.LayuiGreen);
            }
            if (uiTextBoxIn2.Enabled == true)
            {
                ...
                UIMessageBox.Show("月温度数据成功批量合成至年数据！", "提示", UIStyle.LayuiGreen);
            }
            /*
            ...
            info.StrExp = "(b12+b11+b10+b9+b8+b7+b6+b5+b4+b3+b2+b1)/12";
            ...
            if (result)
            {
                UIMessageBox.Show("波段算法执行成功", "提示", UIStyle.LayuiGreen);
            }
            else
            {
